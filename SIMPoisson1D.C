...
bool SIMPoisson1D::parse (char* keyWord, std::istream& is)
{
  ...
  else if (!strncasecmp(keyWord,"MATERIAL",8))
  {
	...
	if (!strncasecmp(cline,"ALL",3))
	  ...
	  {
	    std::cerr <<" *** SIMPoisson1D::parse: Invalid patch index "
		      << patch << std::endl;
	    return false;
	  }
	  std::cout <<"\tMaterial for P"<< patch <<": "<< kappa << std::endl;
	  myProps.push_back(Property(Property::MATERIAL,mVec.size(),patch,1));
	  mVec.push_back(kappa);
	}
    }
  }
  ...
  else if (!strncasecmp(keyWord,"EXNEUMANN",9))
  {
	...
	return false;
      ...
      if (neum.lindx < 1 || neum.lindx > 2)
      {
	std::cerr <<" *** SIMPoisson1D::parse: Invalid vertex index "
